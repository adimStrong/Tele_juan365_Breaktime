"""
Telegram Break Time Tracker Bot - Tele_juan365 Breaktime
...
Break Types:
- ☕ Break (B1/B2) - 1 Hour limit
- 🚻 WC (W1/W2) - 15 mins limit
- ⚠️ Other (O1/O2) - Reason required, no time limit
"""
import os
import pandas as pd
import pytz
...
async def handle_break_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if command not in ['B1', 'B2', 'W1', 'W2', 'O1', 'O2']:
        return
...
Please clock back now using /b2 or /w2"""
...
async def send_daily_report_to_group(context: ContextTypes.DEFAULT_TYPE):
    ...
    report_text += f"\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    report_text += f"🤖 Tele_juan365 Breaktime Bot\n"
...
def main():
    """Start the bot"""
    print("\n" + "="*60)
    print("🤖 Tele_juan365 Break Time Tracker Bot - Production Mode")
    ...
    # Clear bot commands menu (no autocomplete suggestions)
    async def clear_commands(app):
        await app.bot.delete_my_commands()
        print("✅ Bot commands menu cleared (no autocomplete)")

    application.post_init = clear_commands
