...
def init_database():
    ...
    with get_connection() as conn:
        ...
        # Update break type time limits for this project
        try:
            conn.execute("UPDATE break_types SET time_limit_minutes = 60 WHERE code = 'B'")
            conn.execute("UPDATE break_types SET time_limit_minutes = 15 WHERE code = 'W'")
        except Exception as e:
            print(f"Break type update: {e}")
